...
1. When executing the pipeline, depending on the system's hardware one may receive error: 
   ...
   This is because STAR creates lots of temporary files during running and could reach file creation limit by the system. A simple workaround is to set the `threads:` parameter in `rule STAR: (# Step 4-1: Map reads)` to lower values, i.e. 32->16.
